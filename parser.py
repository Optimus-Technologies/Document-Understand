...
#Convert pdf to text
async def convert_pdf_text(document: UploadFile = File(...)):
...
async def convert_docx_text(document: UploadFile = File(...)):
    contents = await document.read()
...
async def convert_image_text(image: UploadFile = File(...)):
...
async def convert_other_text(document: UploadFile = File(...)):
    contents = await document.read()
...
async def document_convertor(ext: str, document: UploadFile = File(...)):
    if ext == "pdf":
        text = await convert_pdf_text(document=document)
    elif ext == "docx":
        text = await convert_docx_text(document=document)
    elif ext in ['jpg', 'jpeg', 'png', 'tiff']:
        text = await convert_image_text(image=document)
    else:
...
async def convert_to_text(document: UploadFile = File(...)):
